# File: ./container.nix
#
# Container building utilities for franken-llama packages.
# Creates slim, portable packages and OCI container images.
{
  ...
  containerConfig = {
    ...
    imageName = config.imageName or "ghcr.io/bowmanjd/llama-cpp";
  ...
  in
    pkgs.runCommand "llama-cpp-slim-${accelType}" {
      ...
      ${lib.optionalString (accelType == "cuda" || accelType == "dual") ''
        # CUDA runtime libraries
        cp -P ${cudaPackages.cuda_cudart}/lib/libcudart.so* $out/lib/
        cp -P ${cudaPackages.libcublas.lib}/lib/libcublas.so* $out/lib/
        cp -P ${cudaPackages.libcublas.lib}/lib/libcublasLt.so* $out/lib/
      ''}
      ...
      ${lib.optionalString (accelType == "vulkan") ''
        # Vulkan runtime libraries
        cp -P ${pkgs.vulkan-loader}/lib/libvulkan.so* $out/lib/
      ''}
      ...
      ${lib.optionalString includeModal ''
        ...
        cp -P ${pythonBase}/lib/libpython3*.so* $out/lib/
        ...
        cp -P ${pkgs.zlib.out}/lib/libz.so* $out/lib/
        cp -P ${pkgs.ncurses.out}/lib/libncursesw.so* $out/lib/
        cp -P ${pkgs.libffi.out}/lib/libffi.so* $out/lib/
        cp -P ${pkgs.expat.out}/lib/libexpat.so* $out/lib/
        cp -P ${pkgs.mpdecimal.out}/lib/libmpdec.so* $out/lib/
        cp -P ${pkgs.sqlite.out}/lib/libsqlite3.so* $out/lib/
        cp -P ${pkgs.readline.out}/lib/libreadline.so* $out/lib/
        cp -P ${pkgs.bzip2.out}/lib/libbz2.so* $out/lib/
        cp -P ${pkgs.xz.out}/lib/liblzma.so* $out/lib/
        cp -P ${pkgs.util-linuxMinimal.lib}/lib/libuuid.so* $out/lib/
      ''}
